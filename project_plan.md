...
Copilot Agent (clarifies → generates SQL → executes → analyzes → responds)
    ↓                        ↓                          ↓
schema.md              MCP SQLite Server          Python / Pandas
...
├── mcp_sql_server.py             # MCP server exposing query/execute/describe
├── data/
...
| 8 | Pick sample dataset + create `.db` file | — | ⬜ (deferred) |
